...
This node is responsible for:
    ...
    - Detecting Apriltags using the camera feed (/tag_detections topic).
    - Transforming detected tag positions to the map frame.
    - Publishing the transformed positions to the goal management node through /detected_tags topic,
      with the tag ID encoded in the orientation.w field.

Since the tag_detections topics contain all the detection at a given time, even if duplicates, the detections are filtered and only the ones
relative to new apriltags found are published to the /detected_tags topic, hence received by the GOAL MANAGEMENT NODE
