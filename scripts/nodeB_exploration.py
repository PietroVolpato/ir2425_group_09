...
class ExplorationNode:
    def __init__(self):
        ...
        self.goal_pub = rospy.Publisher('/exploration_goal', PoseStamped, queue_size=50)
        ...
        self.control_law_pub = rospy.Publisher('/control_law_command', String, queue_size=50)
    ...
    def queue_goal_callback(self, msg):
        ...
        We consider as reached also goals for which the time limit is expired, since as long as the timout was set properly, it is very likely
        that Tiago was not able to reach the goal, despite being close to it, because of some obstructions that deny Tiago to proceed (Tiago occupy
        a non-negligible area).
        A String message is also sent to node A, containing the history of previous 5 goals.
        """
    ...
    def neighborhood_check(self, r, angle, neighborhood_size = 0.3, increment = 0.3):
        ...
        for r_sample in np.arange(r-check_distance, r, increment):
            ...
            delta = 0.05 # spatial frequency of sampling of quare neighborhood
            neighborhood_grid = np.arange(-neighborhood_size, neighborhood_size, delta) # list of offsets w.r.t. center of square

            # compute coordinates of the center of the square defining neighborhood
            x = r_sample * np.cos(angle)
            y = r_sample * np.sin(angle)
            x, y = self.transform_to_map(x, y)

            for dx in neighborhood_grid:
                for dy in neighborhood_grid:
                    if not self.is_free_space(x + dx, y + dy):
                        return False
        return True
    ...
    def random_sample(self, sector, angle_min, angle_increment):
        ...
        while attempts < max_attempts:
            ...
            if not self.is_free_space(x_map,y_map):
                attempts += 1
                continue
    ...
    def explore(self):
        ...
        if not samples:
            ...
            epsilon = 0.1
            self.publish_goal(current_position[0] + epsilon, current_position[1] + epsilon)
            return
