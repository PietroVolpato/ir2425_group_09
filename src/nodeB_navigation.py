...
negligible area in the map.
Then a point (x,y) is inside the "table area" iff x1 <= x <= x2 and y1 <= y <= y2.
...
one on the region bottom left of the table (escape_point_bottom), the other on the region top left of the table (escape_point_top). Such points are computed as function of
x1,x2,y1,y2 and were determined empirically. The proper escape point is given as goal based both on the direction from which Tiago was approaching the table, and on the position of
Tiago w.r.t. the table. There is a good chance that from one escape point Tiago reaches the other (cross the table without collision), resulting in an efficient exploration of the room.
This strategy ensure that Tiago can't collide with the table, despite being an "invisible" obstacle for the lidar sensor.
Despite being a loss of generality, we implemented the coordinates of the table as parameters of the node, in such way that is possible to use this strategy for any rectangular-shape obstacle.
"""
...
class NavigationNode:
    def __init__(self):
        ...
        self.status_pub = rospy.Publisher('/node_b_feedback', String, queue_size=20)
        ...
        self.escape_point_distance = 0.1 + self.safe_distance_table
        self.escape_point_bottom = (self.x2 + self.escape_point_distance, self.y2 - 0.6)
        self.escape_point_top = (self.x2 + self.escape_point_distance, self.y1 + 0.6)
    ...
    def monitor_goal(self):
        """
        This function implements the navigation logic, build for this task (general case more as possible).
        ...
        while not rospy.is_shutdown() and not self.goal_position is None:       
            ...
            delta = 0.2 # do this only when we are relatively close to the table
            # CHOICE OF ESCAPE POINT
            if (self.x1 - delta <= self.current_position[0] <= self.x2 + delta and self.y1 - delta <= self.current_position[1] <= self.y2 + delta):
                if self.current_position[1] > self.y2:  # tiago is in the region below the table
                    approaching_direction = "bottom" # keep track of approaching direction 
                    
                elif self.current_position[1] < self.y1: # tiago is in the region above the table
                    approaching_direction = "top" 
                   
                if self.y1 + self.safe_distance_table <= self.current_position[1] <= self.y2 - self.safe_distance_table:  # tiago aside of the table, and has enough room to go around it
                    # we fix the best escape point in this case
                    if approaching_direction == "bottom":
                        chosen_escape_point = "top"  # tiago is aside the table and was trying to reach upper region
                    else:
                        chosen_escape_point = "bottom" #  tiago is aside the table and was trying to reach lower region
            ...
            if (self.x1 <= self.current_position[0] <= self.x2 and self.y1 <= self.current_position[1] <= self.y2):
                ...
                if chosen_escape_point == "":
                    if approaching_direction == "bottom":
                        chosen_escape_point = "bottom"
                    else:
                        chosen_escape_point = "top"
                self.cancel_goal()
                x_goal = 0
                y_goal = 0
                pose = PoseStamped()
                ...
                if approaching_direction == "top": # want tiago to look below
                    # rotation -90 degrees (facing down)
                    pose.pose.orientation.z = 0.7
                    pose.pose.orientation.w = 0.7
                else: # want tiago to look above
                    pose.pose.orientation.z = -0.7
                    pose.pose.orientation.w = 0.7
                ...
                self.status_pub.publish(String(
                    data=f"DETECTED TABLE PROXIMITY.\nSending 'TABLE ESCAPE GOAL ({chosen_escape_point})': x={x_goal:.3f}, y={y_goal:.3f}."
                ))
                print(f"APPROACHING DIRECTION: {approaching_direction}")
                # Send the goal to move_base
